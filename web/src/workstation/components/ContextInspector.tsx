...
import { TaskOverviewInspector } from "../features/task-overview/TaskOverviewInspector";
import { getWorkstationTaskOverview } from "../features/task-overview/api";
...
export function ContextInspector(): ReactNode {
  ...
  const overviewQuery = useQuery({ queryKey: workstationKeys.detail(taskId ?? ""), queryFn: () => getWorkstationTaskOverview(taskId ?? ""), enabled: taskId !== undefined });
  ...
  if (taskId !== undefined) {
    return (
      <aside className="ny-workstation__inspector" aria-label="上下文检查器">
        {overviewQuery.data === undefined ? <div className="ny-workstation__inspector-state"><p className="ny-workstation__inspector-task-reference">任务 {taskId}</p><p>详细进度、产物和恢复操作将<span className="ny-workstation__inspector-copy-phrase">在<span className="ny-workstation__inspector-copy-phrase">此处显示</span></span>。</p></div> : <TaskOverviewInspector selectedStage={searchParams.get("stage") ?? overviewQuery.data.current_stage} task={overviewQuery.data} />}
      </aside>
    );
  }
  ...
  return (
    <aside className="ny-workstation__inspector" aria-label="上下文检查器">
      ...
      <h2 className="ny-workstation__inspector-heading">工作台状态</h2>
